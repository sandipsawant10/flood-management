...
const RescuerDashboard = () => {
  ...
  const { data: emergencies, isLoading: emergenciesLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        "/api/emergency/active?teamId=" + user.rescueTeam
      );
  ...
  const { data: teamMembers, isLoading: membersLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        "/api/emergency/team/" + user.rescueTeam + "/members"
      );
  ...
  const updateMemberStatus = useMutation({
    mutationFn: async ({ memberId, status }) => {
      const response = await fetch(
        `/api/emergency/team/member/${memberId}/status`,
        {
          method: "PUT",
  ...
  const renderEmergencies = () => {
    if (emergenciesLoading)
      return <div className="p-4">Loading emergencies...</div>;
    if (!emergencies?.length)
      return <div className="p-4">No active emergencies.</div>;
        ...
        {emergencies.map((emergency) => (
          ...
          >
            ...
            <div className="mt-2 space-y-2 text-sm text-gray-600">
              ...
              <p>
                <strong>Location:</strong> {emergency.location.address}
              </p>
              <p>
  ...
  const renderTeamMembers = () => {
    ...
    if (!teamMembers?.length)
      return <div className="p-4">No team members found.</div>;
