...
const MunicipalityDashboard = () => {
  ...
  const updateAlertMutation = useMutation({
    mutationFn: async ({ alertId, status }) => {
      const { data } = await axiosInstance.patch(`/api/alerts/${alertId}`, {
        status,
      });
      return data;
    },
  ...
  // Stats calculation - ensure data is array before filtering
    ...
    <Box p={4}>
      ...
      <Tabs variant="enclosed">
        ...
        <TabPanels>
          ...
          {/* Analytics Panel */}
          <TabPanel>
            <AnalyticsPanel reports={reportsArray} alerts={alertsArray} />
          </TabPanel>
        </TabPanels>
      </Tabs>
    </Box>
              ...
              <CardBody>
                <VStack align="stretch" spacing={4}>
                  ...
                  <Box>
                    ...
                    <Text>
                      Affected People:{" "}
                      {selectedReport.impact?.affectedPeople || "N/A"}
                      <br />
                      Damaged Properties:{" "}
                      {selectedReport.impact?.damagedProperties || "N/A"}
                    </Text>
                  </Box>
...
const AnalyticsPanel = ({ reports, alerts }) => {
  ...
  const reportStats = {
    byStatus: {
      ...
      verified: reports.filter((r) => r.verificationStatus === "verified")
        .length,
      disputed: reports.filter((r) => r.verificationStatus === "disputed")
        .length,
    },
    bySeverity: {
        ...
        <Card>
          <CardBody>
            ...
            <Grid templateColumns="repeat(2, 1fr)" gap={4}>
              {Object.entries(reportStats.bySeverity).map(
                ([severity, count]) => (
                  <Box
                    key={severity}
                    textAlign="center"
      ...
      <Box>
        ...
        <Card>
          <CardBody>
            ...
            <Grid templateColumns="repeat(2, 1fr)" gap={4}>
              {Object.entries(alertStats.bySeverity).map(
                ([severity, count]) => (
                  <Box
                    key={severity}
                    textAlign="center"
