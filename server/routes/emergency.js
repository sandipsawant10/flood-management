...
router.get('/active', authorize(['rescuer', 'admin']), async (req, res) => {
  try {
...
// Get team members
router.get('/team/:teamId/members', authorize(['rescuer', 'admin']), async (req, res) => {
  try {
...
router.put('/team/member/:memberId/status', authorize(['rescuer', 'admin']), async (req, res) => {
  try {
    const { status } = req.body;
    const member = await RescueTeam.findOneAndUpdate(
      { 'members._id': req.params.memberId },
      { $set: { 'members.$.status': status, 'members.$.lastUpdate': new Date() } },
      { new: true }
    );
    if (!member) {
...
// Get team resources
router.get('/team/:teamId/resources', authorize(['rescuer', 'admin']), async (req, res) => {
  try {
...
router.put('/team/resource/:resourceId', authorize(['rescuer', 'admin']), async (req, res) => {
  try {
...
router.put('/:emergencyId/status', authorize(['rescuer', 'admin']), async (req, res) => {
  try {
