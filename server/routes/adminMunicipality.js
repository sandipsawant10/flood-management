...
router.patch('/reports/:reportId', auth, roleAuth('municipality'), async (req, res) => {
  try {
...
router.post('/alerts', auth, roleAuth('municipality'), async (req, res) => {
  try {
    const { title, message, severity, targetArea, audience, duration } = req.body;
...
router.get('/analytics', auth, roleAuth('municipality'), async (req, res) => {
  try {
    ...
    res.json({
      reportStats: reportStats.reduce((acc, curr) => ({ ...acc, [curr._id]: curr.count }), {}),
      alertStats: alertStats.reduce((acc, curr) => ({ ...acc, [curr._id]: curr.count }), {})
    });
  } catch (error) {
